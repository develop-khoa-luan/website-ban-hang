...
<div class="features_items">
	...
	<h2 class="title text-center" style="margin-top: 10px">giá từ cao đến thấp</h2>
	@foreach($all_product_desc as $key => $product_desc)
	<form>
		<div class="col-sm-4">
			<div class="product-image-wrapper">
				<input name="product_id_hidden" id="product_id" type="hidden" value="{{$product_desc->product_id}}" />
				<input name="qty" type="hidden" min="0" value="1" />
				<a href="{{URL::to('/chi-tiet-san-pham/'.$product_desc->product_id)}}">
					<div class="single-products">
						<div class="productinfo text-center">
							<img style="height:250px" src="public/uploads/product/{{$product_desc->product_image}}" alt="" />
							<h2>{{number_format($product_desc->product_price).' '.'VND'}}</h2>
							<p><strong>{{$product_desc->product_name}}<strong></strong></p>
							<p>{{$product_desc->brand_name}} </p>
							{{-- <p>{{$product->category_name}} </p> --}}
							<button class="btn btn-default show-modal-product-detail" data-toggle="modal"
								data-target="#{{$product_desc->product_id}}-modal">
	...
	@endforeach
	<div class="col-sm-12 text-center"> {{$all_product->links()}}</div>
